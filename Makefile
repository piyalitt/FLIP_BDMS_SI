...
# Read the internal service key from the key file (generated by `make generate-internal-service-key`).
# fl-server uses this to authenticate with flip-api. The key file is the source of truth;
# INTERNAL_SERVICE_KEY is no longer stored in .env files.
override INTERNAL_SERVICE_KEY := $(shell cat deploy/keys/internal_service.key 2>/dev/null)
