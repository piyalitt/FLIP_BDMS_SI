...
Go to pgAdmin (<http://localhost:5050>) and log in (note the port and credentials are defined in [.env.development](../../.env.development), as `PGADMIN_PORT`, `PGADMIN_EMAIL`, and `PGADMIN_PASSWORD`).

Click on Register Server in the pgAdmin interface and configure the connection:

- Under 'General' tab
  - Name: trust (or any name you prefer)
- Under 'Connection' tab
  - Host name/address: `omop-db`
  - Port: `5432`
  - Username: `OMOP_POSTGRES_USER` defined in [.env.development](../../.env.development) under `OMOP database credentials`
  - Password: `OMOP_POSTGRES_PASSWORD` defined in [.env.development](../../.env.development) under `OMOP database credentials`
  - Toggle 'Save password'

To view the data you've uploaded, on pgAdmin: Right click on the table (e.g. `image_occurrence`) > `Scripts` > `SELECT Script` and then execute the script.

You can import csv data using pgAdmin also, but it is not recommended.

## Dev notes

**Note**: currently we are not using the encrypt service of the data-import-api to encrypt the accession IDs before going into the OMOP database.

### SQL initialization

Had to add the below in the sql initialization script that I exported from the flip mock trust db using 'Backup'. This was so that the `serviceaccount` was created with no errors.

```sql
-- Create service account user for OMOP database
CREATE USER serviceaccount WITH ENCRYPTED PASSWORD 'OMOP_SERVICE_PASSWORD';

--
-- TOC entry 3730 (class 0 OID 0)
-- Dependencies: 6
-- Name: SCHEMA omop; Type: ACL; Schema: -; Owner: postgres
--
...
To run the tests locally, you need to have the database running. You can do this by running:
