...
class TestCORSConfiguration:
    ...
    def test_cors_middleware_uses_explicit_allowlist_not_wildcard(self):
        """CORSMiddleware must be registered with the configured allowlist, never ['*']."""
        ...
        assert kwargs["allow_origins"] != ["*"], "Wildcard origins are unsafe with allow_credentials=True"

    def test_allowed_origin_is_reflected_with_vary_header(self):
        ...
        # Bare TestClient (no context manager) skips the lifespan/scheduler — we only need middleware.
    ...
    def test_disallowed_origin_does_not_receive_acao(self):
